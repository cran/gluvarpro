        ...
        for(i in 1:length(serie)) {
                ...
                if(!any(is.na(aux$glucose))) {
                        ...
                        if(n >= 24) {
                                ...
                                x$mv[position] <- round(mean(abs(log10(as.numeric(as.character(aux$glucose)) / gi)) ^ 3, na.rm = TRUE), digits = 4)

                        } else {

                                w <- (max(aux$glucose) - min(aux$glucose)) / 20
                                x$mv[position] <- round(mean(abs(log10(as.numeric(as.character(aux$glucose)) / gi)) ^ 3, na.rm = TRUE) + w, digits = 4)
