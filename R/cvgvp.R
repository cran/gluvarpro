...
cvgvp <- function(x, t = 24) {
        ...
        for(i in 1:length(serie)) {
                ...
                if(!all(is.na(aux$glucose))) {
                        ...
                        x$cv[position] <- round(sd(as.numeric(as.character(aux$glucose)), na.rm = TRUE) / mean(as.numeric(as.character(aux$glucose)), na.rm = TRUE), digits = 4) * 100
